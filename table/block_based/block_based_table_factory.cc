...
static struct BlockBasedTableTypeInfo {
  ...
  BlockBasedTableTypeInfo() {
#ifndef ROCKSDB_LITE
    info = {
        // NOTE: almost all of these are marked mutable below because there
        // should be no danger in mutability, as table builders and readers
        // work from copies of BlockBasedTableOptions.
    ...
    for (auto& i : info) {
      // Assume mutable because there should be no danger in mutability, as
      // table builders and readers work from copies of BlockBasedTableOptions.
